...
export function isBlockedIpv6(address: string): boolean {
  ...
  // Ambiguous IPv4-mapped, NAT64, local, multicast, and reserved forms are
  // unnecessary for this product. Fail closed instead of relying on platform
  // translation behavior.
  if (ipv6Prefix(value, "::ffff:0:0", 96)) return true;
...
export function parseTargetUrl(rawUrl: string): URL {
  ...
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw new UrlFormatError("http / https URLのみ監査できます。");
  }
  if (url.username || url.password) {
    throw new TargetPolicyError("認証情報を含むURLは監査できません。");
  }
  if (url.port && url.port !== "80" && url.port !== "443") {
    throw new TargetPolicyError("安全のため80 / 443番ポートのみ監査できます。");
  }
  ...
  if (
    ...
    throw new TargetPolicyError("localhost または内部Networkの宛先は監査できません。");
  }

  if ((isIPv4(hostname) || isIPv6(hostname)) && !isPublicAddress(hostname)) {
    throw new TargetPolicyError("Private / link-local / reserved IPへのアクセスをブロックしました。");
  }
